...
(defun nearest-overlap (wire-1 wire-2)
  (let* ((origin (make-coord :row 0 :col 0))
         (path-1 (run-wire (list origin) wire-1))
         (path-2 (run-wire (list origin) wire-2)))
    (let ((result '())
          (dist 0))
      ;; Scan path-2 to see where it collides with path-1
      (format t "Starting search - initial path ~A long~%" (length path-1))
      (dolist (p2 path-2)
        (when (member p2 path-1 :test #'coord-eq)
          (let ((pdist (manhattan-distance p2)))
            (unless (zerop pdist)
              (when (or (null result)
                        (< pdist dist))
                (format t "Candidate ~A distance ~A~%" p2 pdist)
                (setf result p2)
                (setf dist pdist))))))
      (values dist result))))
...
;;;; Test data
(defvar origin (make-coord :row 0 :col 0))
...
(defun wire-bounds (wire-path)
  (let ((minrow 0) (mincol 0)
                   (maxrow 0) (maxcol 0))
    (declare (fixnum minrow mincol maxrow maxcol))
    (dolist (c wire-path)
      (declare (coord c))
      (setf maxrow (max maxrow (coord-row c)))
      (setf minrow (min minrow (coord-row c)))
      (setf maxcol (max maxcol (coord-col c)))
      (setf mincol (min mincol (coord-col c))))
    (list minrow mincol maxrow maxcol)))

(defun wire-end (wire)
  "Test function - returns the final displacement of a wire"
  (let ((row 0) (col 0))
    (dolist (w wire)
      (format t "~A " w)
      (case (direction-ordinal w)
        (#\U (decf row (direction-distance w))) 
        (#\D (incf row (direction-distance w)))
        (#\L (decf col (direction-distance w)))
        (#\R (incf col (direction-distance w)))))
    (cons row col)))
