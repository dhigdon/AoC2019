...
(defun parse-fixnum (s)
  (declare (string s))
  (the fixnum (parse-integer s)))

(defun load-icode (s)
  (declare (string s))
  ;; coerce seems to be more performant than (apply #'vector ...)
  (coerce (mapcar #'parse-fixnum (cl-ppcre:split "," s))
          'vector))

(defun run (icode)
  (declare (simple-vector icode))
  (do ((pc 0 (+ pc 4))) ; opcodes increment PC themselves
    ((= (svref icode pc) 99) icode)
    ;; Parse the opcode
    (let* ((op (svref icode pc))
           (p1 (svref icode (+ pc 1)))
           (p2 (svref icode (+ pc 2)))
           (res (svref icode (+ pc 3))))
      ;; Evaluate the opcode
      (setf (svref icode res)
            (case op
              (1 (+ (svref icode p1)
                    (svref icode p2)))
              (2 (* (svref icode p1)
                    (svref icode p2)))
              ;; TODO: work out how to check op 99 here!
              (otherwise (error "Bad opcode ~A at address ~A.~%" op pc)))))))
